...
///////////////////////////////////////////////////////////
class crearGraficoPie extends crearGrafico {
	...
	var $pie_3D_height_percent = 8; //is % of total width 
	...
	var $pie_avail_colors = array('azul_juassi','pastel_orange_1','pastel_orange_2','pastel_blue_1','pastel_green_1','clay',
	...
	var $pie_3D_height;
	function crearGraficoPie($width = '', $height = '',$output_file = NULL) {
		crearGrafico::crearGrafico($width, $height, $output_file);	
	}
	function inicializar() {
		crearGrafico::inicializar();
	...
	function generarLeyenda() {
		...
		foreach ($this->data_array[0] as $dataText => $dataValue) {
			...
			//DRAW COLOR BOXES
			$color = $this->generarProximoColor();
	...
	function generarBarras()
	{
		...
		for($i = $this->pie_center_y + $this->pie_3D_height; $i > $this->pie_center_y; $i--) {
			...
			foreach ($this->pie_data_array_percents as $key => $value) {
				$color = $this->generarProximoColor(true); //GENERATE A DARKER VERSION OF THE INDEXED COLOR
		...
		foreach ($this->pie_data_array_percents as $key => $value) {
			...
			if ($this->bool_data_labels) { $this->generarDataLabel($value, $arcStart); }
			$arcStart += 360 * $value;
		}
	}
	function generarDataLabel($value, $arcStart) {
	...
	function generarProximoColor($dark = false) {
		$array = $this->retornarColores($this->pie_avail_colors[$this->pie_color_pointer]);
		if ($dark) {
	...
	function retornarColores($color) {
		...
		if ($resultColor = crearGrafico::retornarColores($color)) {
			return $resultColor;
		}
		else {
			...
			switch(strtolower($color)) {
				//NAMED COLORS BASED ON W3C's RECOMMENDED HTML COLORS
                                case 'azul_juassi': return array(0,109,141); break;
