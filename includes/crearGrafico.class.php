...
class crearGrafico{
	...
	function crearGrafico($width = '', $height = '', $output_file = NULL) {
		//verificamos que se ha suministrado unas medidas, sino es asi, le asignamos una por defecto
		...
		$this->inicializar();
		$this->asignarColores(); //configuramos los colores por defecto	
	}
	
	function inicializar() {
		//el header debe ser enviada antes de cualquier salida html o espacio en blanco
	...
	function dibujarGrafico() {
		//metodo principal de la clase - debe ser llamado de último llamado de último
		//configuramos las axixas si el usuario no las ha configurado
		if ($this->bool_data){
			if (!$this->bool_x_axis_setup){ $this->configurarXAxis(); }
			if (!$this->bool_y_axis_setup){ $this->configurarYAxis(); }
			//calculos
			$this->calcularTopMargen();
			$this->calcularMargenDerecho();
			$this->calcularCoordenadas();
			$this->configurarData();
			//start creating actual image elements
			if ($this->bool_background){ $this->generarBackgound(); }
			//always gen grid values, even if not displayed
			$this->generarGrid();
			if ($this->bool_bars_generate){ $this->generarBarras(); }
			if ($this->bool_data_points){ $this->generarDataPoints(); }
			if ($this->bool_legend){ $this->generarLeyenda(); }
			if ($this->bool_title){ $this->generarTitulo(); }
			if ($this->bool_x_axis){ $this->generarXAxis(); }
			if ($this->bool_y_axis){ $this->generarYAxis(); }
		}
		else {
	...
	function configurarXAxis($percent = '', $color = '') {
		if ($percent === false) {
		...
		if (!empty($color) && $arr = $this->retornarColores($color)) {
	...
	function configurarYAxis($percent = '', $color = '') {
		if ($percent === false) {
	...
	function retornarColores($color) {
		//chequeamos que los valores pasados en el formulario son numeros: '128,128,128'
		if (strpos($color,',') !== false) {
			return explode(',', $color);
		}
		//chequeamos a ver si son colores hex
		...
		switch (strtolower($color)) {
			//Colores con nombres BASADOS EN COLORES RECOMENDADOS HTML4.0 de W3C
			...
			//Colores con nombres BASADOS EN COLORES RECOMENDADOS HTML5 de W3C
			case 'aliceblue':   return array(240,248,255); break;
			case 'antiquewhite':   return array(250,235,215); break;
			case 'aquamarine':   return array(127,255,212); break;
			case 'azure':   return array(240,255,255); break;
			case 'beige':   return array(245,245,220); break;
			case 'bisque':   return array(255,228,196); break;
			case 'blanchedalmond':   return array(255,235,205); break;
			case 'blueviolet':   return array(138,43,226); break;
			case 'brown':   return array(165,42,42); break;	
			case 'burlywood':   return array(222,184,135); break;
			case 'cadetblue':   return array(95,158,160); break;
			case 'chartreuse':   return array(127,255,0); break;
			case 'chocolate':   return array(210,105,30); break;
			case 'coral':   return array(255,127,80); break;
			case 'cornflowerblue':   return array(100,149,237); break;
			case 'cornsilk':   return array(255,248,220); break;
			case 'crimson':   return array(220,20,60); break;
			case 'darkblue':   return array(0,0,139); break;
			case 'darkcyan':   return array(0,139,139); break;
			case 'darkgoldenrod':   return array(184,134,11); break;
			case 'darkgray':   return array(169,169,169); break;
			case 'darkgreen':   return array(0,100,0); break;
			case 'darkkhaki':   return array(189,183,107); break;
			case 'darkmagenta':   return array(139,0,139); break;
			case 'darkolivengreen':   return array(85,107,47); break;
			case 'darkorange':   return array(255,140,0); break;
			case 'darkorchid':   return array(153,50,204); break;
		}
		$this->error[] = "Color name \"$color\" not recogized.";
		return false;
	}
	
	function calcularTopMargen() {
		if ($this->bool_title) {
			//incluimos espacio para el titulo, margen aproximado + 3*alto titulo
			$this->top_margin = ($this->height * ($this->x_axis_default_percent / 100)) + $this->title_char_height;
		}
		else {
			//solo usamos el espacio por defecto
			$this->top_margin = $this->height * ($this->x_axis_default_percent / 100);
		}	
	}
	
	
	function calcularMargenDerecho() {
		//solo usamos el espacio por defecto
	...
	function calcularCoordenadas() {
		//calculamos los puntos de las axixas, tambien usados por otros calculos
	...
	function configurarData() {
		...
		if ($unit_width < 1) {	
			...
			$this->error[] = "Grafico demasiado pequeno o demasiado puntos de Data.";
		}
		else {
	...
	function generarGrid() {
		...
		$this->calcularGridHorizontal($adjustment);
		$this->calcularGridVertical();
		//output values - separation from calc is b/c of an extension related need
		$this->generarGrids();
		$this->generarLineaGuia($adjustment);
	}
	
	
	
	function calcularGridHorizontal($adjustment = 0) {
		...
		foreach ($horizGridArray as $value) {
			...
			if ($this->bool_y_axis_values) {
				...
				if ($this->data_format_array) {
					$value = $this->aplicarFormatoData($value);
				}
				if ($this->data_currency) {
					$value = $this->aplicarActualData($value);
				}
	...
	function generarLineaGuia($adjustment = 0) {
			...
			foreach ($data_set as $key => $item) {
				...
				if ($this->bool_bars) {
					if ($this->bool_gradient) {
						...
						$this->dibujarDegradeBarras($x1, $y1, $x2, $y2, $this->multi_gradient_colors_1[$data_set_num], $this->multi_gradient_colors_2[$data_set_num], $data_set_num);
					}
					else {
				...
				if ($this->bool_data_values) {
					...
					//add currency sign, formatting etc
					if ($this->data_format_array) {
						$item = $this->aplicarFormatoData($item);
					}
					if ($this->data_currency) {
						$item = $this->aplicarActualData($item);
					}
	...
	function aplicarFormatoData($input) {
	...
	function aplicarActualData($input) {
		return $this->data_currency . $input;
	}
	
	function generarDataPoints() {
	...
	function mostrarErrores() {
		if (count($this->error) > 0) {
			...
			imagestring($this->image, 3, 2, 0, "!!----- crearGrafico Error -----!!",  $errorColor);
	...
	function agregarData($data, $data2 = '', $data3 = '', $data4 = '', $data5 = '') {
		...
		if ($this->data_set_count == 0) {
			...
			$this->calcularMinMax($min, $max);
		}
	}
	
	
	function calcularMinMax($min, $max) {
		$this->bool_data = true;
	...
	function dibujarDegradeBarras($x1, $y1, $x2, $y2, $colorArr1, $colorArr2, $data_set_num) {
	...
	function setGenericColor($inputColor, $var, $errorMsg) {
		...
		if (!empty($inputColor) && ($arr = $this->retornarColores($inputColor))) {
	...
	function setGradient($color1, $color2, $color3='', $color4='', $color5='', $color6='', $color7='', $color8='', $color9='', $color10='') {
		if (!empty($color1) && !empty($color2) && ($arr1 = $this->retornarColores($color1)) && ($arr2 = $this->retornarColores($color2))) {
		...
		//Optional gradients
		if (!empty($color3) && !empty($color4) && ($arr1=$this->retornarColores($color3)) && ($arr2 = $this->retornarColores($color4))) {
			$this->bool_gradient=true;
			$this->multi_gradient_colors_1[]=$arr1;
			$this->multi_gradient_colors_2[]=$arr2;
		}
		if (!empty($color5) && !empty($color6) && ($arr1 = $this->retornarColores($color5)) && ($arr2 = $this->retornarColores($color6))) {
			$this->bool_gradient = true;
			$this->multi_gradient_colors_1[] = $arr1;
			$this->multi_gradient_colors_2[] = $arr2;
		}
		if (!empty($color7) && !empty($color8) && ($arr1 = $this->retornarColores($color7)) && ($arr2 = $this->retornarColores($color8))) {
			$this->bool_gradient = true;
			$this->multi_gradient_colors_1[] = $arr1;
			$this->multi_gradient_colors_2[] = $arr2;
		}
		if (!empty($color9) && !empty($color10) && ($arr1 = $this->retornarColores($color9)) && ($arr2 = $this->retornarColores($color10))) {
			$this->bool_gradient = true;
			$this->multi_gradient_colors_1[] = $arr1;
			$this->multi_gradient_colors_2[] = $arr2;
		}
