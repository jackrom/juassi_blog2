...
juassi_set_admin_title('Approve Comment');
...
if (juassi_user_can('edit_comments')) {
	if (isset($_REQUEST['comment_id']) && (int)$_REQUEST['comment_id'] != 0) {
		...
		if (isset($_POST['submit']) && $_POST['submit'] == 'Approve') {
			...
			$message = '<strong>Comment approved.</strong>';
...
<?php
	...
	if (isset($juassi_comment)) {
		...
		?>
		<p>Are you sure you wish to approve this comment?</p>
		<div class="comments">
			<cite>
		...
		<form method="post" action="<?php echo juassi_htmlentities($_SERVER['PHP_SELF']); ?>">
			<p><input type="hidden" name="comment_id" value="<?php echo juassi_comment_id(); ?>" /><input type="submit" name="submit" value="Approve" /></p>
		</form>
		<br />
		<?php
	}
