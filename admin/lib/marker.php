...
class juassi_marker {
  ...
  function juassi_counter_file($cache_path, $counter_pre, $counter_suf) {
    global $JUASSI_COUNTER_FILES;

    mt_srand((double) microtime() * 1000000);
    return ($cache_path.$counter_pre.mt_rand(0, ($JUASSI_COUNTER_FILES - 1)).$counter_suf);
  }

  function juassi_known_range($addr, $class_a) {
    // look up whether an address is registerred
    global $JUASSI_IP2EXT_PATH;

    $long = sprintf("%u", ip2long($addr));
    $file = $JUASSI_IP2EXT_PATH.$class_a.".inc";
    $is_valid = false;
  ...
  function juassi_valid_ip($addr, $prx = 0) {
    $iptest = explode(".", $addr);
  ...
  function juassi_select_host($array) {
    arsort($array, SORT_NUMERIC);

    foreach ($array as $key => $val) {
      $key = $this->juassi_hex2ip(trim($key));

      if (($prx = $this->juassi_valid_ip($key, 1)) !== false) return $prx;
    }
    return false;
  }

  // extract the first valid address from a chain
  function juassi_unchain_addr($val) {
    if (strpos($val, ",") === false) return $val;

    $array = explode(",", $val);

    for ($i = 0, $max = count($array); $i < $max; $i++) $array[$i] = trim($array[$i]);
    return $this->juassi_select_host(array_flip($array));
  }

  // return the correct remote address
  function juassi_get_remote_addr($addr, $reverse) {
    $addr = $this->juassi_unchain_addr($addr);
    $reverse = $this->juassi_unchain_addr($reverse);

    if ((!empty($reverse)) && ($this->juassi_valid_ip($addr, 1) === false)) return $reverse;
  ...
  function juassi_parse_headers() {
    if (_JUASSI_PHP < 410) global $HTTP_SERVER_VARS;

    foreach (((_JUASSI_PHP < 410) ? $HTTP_SERVER_VARS : $_SERVER) as $key => $val) {
    ...
    return (!empty($chk) ? $this->juassi_select_host($chk) : false);
  }

  // Check if an ip address is matching up against the blacklist
  function juassi_is_ignored($blacklist, $client) {
  ...
  function juassi_ignore_ref($array) {
    global $JUASSI_IGNORE_REFER;

    if (!empty($JUASSI_IGNORE_REFER)) {
      foreach(explode(",", $JUASSI_IGNORE_REFER) as $test) {
        $test = trim($test);
  ...
  function juassi_parse_ref($ref) {
    if (!$this->juassi_valid_ref($ref)) return false;
  ...
  function juassi_filter_ref($srvhost, $ref, $srvname, $srvaddr) {
    $ref_array = $this->juassi_parse_ref($ref);

    if (is_array($ref_array) && ($this->juassi_ignore_ref($ref_array) !== false)) return "ignored";
  ...
  function juassi_filter_uri($script, $pinfo, $uri) {
    // getting rid of stupid user input
  ...
  function juassi_auto_page_name($uri) {
  ...
  // write the entry
  function juassi_write_entry() {
    global $JUASSI_CACHE_PATH;
    ...
    $fp = defined("_JUASSI_DIO") ? dio_open($file, O_RDWR | O_APPEND) : fopen($file, "ab+");
    ...
    if (defined("_JUASSI_DIO") && (dio_fcntl($fp, F_SETLK, 1) !== -1)) {
      ...
      if (defined("_JUASSI_SEM") ? ($id = juassi_semlock($file)) : flock($fp, LOCK_EX)) {
        fputs($fp, $this->string);
        fflush($fp);
        defined("_JUASSI_SEM") ? sem_release($id) : flock($fp, LOCK_UN);
  ...
  // constructor
  function juassi_marker() {
    if (_JUASSI_PHP < 410) global $HTTP_SERVER_VARS;

    global $JUASSI_CACHE_PATH, $JUASSI_COUNTER_PREFIX, $JUASSI_COUNTER_SUFFIX, $JUASSI_IGNORE_IP, $JUASSI_SEP, $JUASSI_TIMESTAMP,
           $JUASSI_TIME_OFFSET, $DOCUMENT_ROOT, $HTTP_HOST, $HTTP_X_REMOTECLIENT_IP, $LOCAL_ADDR, $PATH_INFO,
           $PHP_SELF, $SCRIPT_FILENAME, $SERVER_NAME;

    $this->sep = $JUASSI_SEP;
    $this->ignored = false;
    $this->filename = $this->juassi_counter_file($JUASSI_CACHE_PATH, $JUASSI_COUNTER_PREFIX, $JUASSI_COUNTER_SUFFIX);

	$time = $JUASSI_TIMESTAMP + ($JUASSI_TIME_OFFSET * 60);
    ...
    foreach ($hdr as $str) {
      $str = ((_JUASSI_PHP < 410) ? !empty($HTTP_SERVER_VARS[$str]) : !empty($_SERVER[$str])) ?
               juassi_clean(((_JUASSI_PHP < 410) ? $HTTP_SERVER_VARS[$str] : $_SERVER[$str]), $JUASSI_SEP) : false;
    }
    ...
    $REMOTE_ADDR = ((stristr(PHP_OS, "darwin") !== false) && !empty($HTTP_PC_REMOTE_ADDR)) ? $HTTP_PC_REMOTE_ADDR :
                   $REMOTE_ADDR;
    $REQUEST_URI = $this->juassi_filter_uri($filename, $PATH_INFO , $PHP_SELF);
    $SERVER_ADDR = empty($SERVER_ADDR) ? $LOCAL_ADDR : $SERVER_ADDR;
    $SERVER_ADDR = $this->juassi_valid_ip($SERVER_ADDR) ? $SERVER_ADDR : "127.0.0.1";
    $HTTP_USER_AGENT = empty($HTTP_USER_AGENT) ? "unknown" : $HTTP_USER_AGENT;
    $HTTP_REFERER = empty($HTTP_REFERER) ? "unknown" :
                    $this->juassi_filter_ref($HTTP_HOST, $HTTP_REFERER, $SERVER_NAME, $SERVER_ADDR);
    // Use a page name even if the user didn't specify it
    $page = defined("_JUASSI_PAGE_NAME") ? juassi_clean(_JUASSI_PAGE_NAME, $BBC_SEP) : $this->juassi_auto_page_name($REQUEST_URI);
    $prx = $this->juassi_parse_headers();

    if (!empty($prx)) {
      $prx_addr = $this->juassi_get_remote_addr($REMOTE_ADDR, $HTTP_X_REMOTECLIENT_IP);

      if (($this->ignored = $this->juassi_is_ignored($JUASSI_IGNORE_IP, $prx_addr)) !== false) return;
      else $REMOTE_ADDR = juassi_clean($prx, $BBC_SEP);
    }
    else {
      $prx_addr = "unknown";
      $REMOTE_ADDR = $this->juassi_get_remote_addr($REMOTE_ADDR, $HTTP_X_REMOTECLIENT_IP);
    }

    if (($this->ignored = $this->juassi_is_ignored($JUASSI_IGNORE_IP, $REMOTE_ADDR)) !== false) return;
