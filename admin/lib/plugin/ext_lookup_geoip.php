...
function juassi_extension_plugin($host, $addr) {
    global $JUASSI_GEOIP_PATH, $gi;

    // First of all let's check if the include file exists
    if (!@file_exists($JUASSI_GEOIP_PATH.'geoip.inc')) {
        juassi_msg('Missing geoip installation');
    }
    include_once($JUASSI_GEOIP_PATH.'geoip.inc');
...
if (preg_match('/^.{1,4}:/',$addr)) {
    $gi = geoip_open($JUASSI_GEOIP_PATH ."GeoIPv6.dat",GEOIP_STANDARD);
    $addr = geoip_country_code_by_addr_v6($gi, $addr);
 } else {
    $gi = geoip_open($JUASSI_GEOIP_PATH ."GeoIP.dat",GEOIP_STANDARD);
