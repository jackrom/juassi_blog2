...
// referer stats
function juassi_sum_item($keep, $remove) {
  global $access;
...
function juassi_update_referer_stat($referer) {
  global $access;
  ...
  elseif (!isset($access['referer'][$referer_light]) && isset($access['referer'][$result])) {
    isset($access['referer'][$res_no_slash]) ? juassi_sum_item($result, $res_no_slash) : "";

    ++$access['referer'][$result];
  }
  // same here...
  elseif (isset($access['referer'][$referer_light]) && !isset($access['referer'][$result])) {
    isset($access['referer'][$res_no_slash]) ? juassi_sum_item($referer_light, $res_no_slash) : "";
  ...
  // Now we got both of them, let's continue with the one we got most of
    ...
    if ($access['referer'][$referer_light] < $access['referer'][$result]) {
      juassi_sum_item($result, $referer_light);

      ++$access['referer'][$result];
    }
    else {
      juassi_sum_item($referer_light, $result);
...
function juassi_fix_refer_stat($refsum) {
  global $access;
...
function juassi_get_sep($query, $array) {
...
function juassi_get_search($array) {
...
function juassi_get_keywords($ref) {
  global $JUASSI_CUSTOM_CHARSET, $access;
  ...
  $get_vars = juassi_get_sep($ref, $var_sep);
  $raw_search = juassi_get_search($get_vars);

  if ($raw_search === false) return false;

  // Conversion of keywords, if applicable
  $from = extension_loaded("mbstring") ? juassi_get_encoding($raw_search) : false;
  $char = (!empty($JUASSI_CUSTOM_CHARSET)) ? $JUASSI_CUSTOM_CHARSET : false;
  $raw_search = (($from !== false) || extension_loaded("recode")) ?
                juassi_convert_lang($raw_search, $from, $char) : $raw_search;
  $flt_search = juassi_get_sep($raw_search, $word_sep);
  ...
  for ($i = 0, $j = count($flt_search); $i < $j; $i++) {
    ...
    $flt_search[$i] = (!$char || (stristr($char, "UTF") === false)) ?
                      strtolower(bbc_clean($flt_search[$i])) : juassi_clean($flt_search[$i]);
