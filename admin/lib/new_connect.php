...
function juassi_get_extension($host, $addr) {
	global $JUASSI_EXT_LOOKUP;
	
	$ext = "";
	if ($JUASSI_EXT_LOOKUP) {
		// if plugin is defined, then try lookup via plugin
		$ext = juassi_extension_plugin($host, $addr);
	}
...
function juassi_update_connect($connect) {
  global $JUASSI_LIB_PATH;

  // Sanity check has already been made in mark_page.php
  foreach (array("browser", "os", "robot") as $i) require($JUASSI_LIB_PATH.$i.".php");
...
function juassi_update_access($connect) {
  global $access;
