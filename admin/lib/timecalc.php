...
function juassi_time_offset() {
  global $JUASSI_TIMESTAMP, $JUASSI_TIME_OFFSET, $access, $last;

  $cfg_offset = !empty($JUASSI_TIME_OFFSET) ? $JUASSI_TIME_OFFSET : 0;
  ...
  $new_time = $JUASSI_TIMESTAMP + ($cfg_offset * 60);
  $old_time = $JUASSI_TIMESTAMP + ($sav_offset * 60);
...
function juassi_update_time_stat($new_time, $last_time) {
  global $access;
